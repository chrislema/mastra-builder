...
function renderWorkerSmokeTest(language: ProjectLanguage) {
  ...
  return [
    ...
    '  });',
    '',
    "  it('serves static assets through the ASSETS fallback', async () => {",
    "    const request = new Request('https://example.com/');",
    "    const assets = { fetch: async () => new Response('asset shell', { status: 200 }) };",
    `    const response = await worker.fetch(request, { ASSETS: assets }${castEnv}, {}${castCtx});`,
    '    expect(response.status).toBe(200);',
...
export function renderProjectScaffold(input: unknown): ProjectScaffold {
  ...
  const manifest = {
    ...
    bindingMap: bindingMapForProfiles(profiles, externalServiceBindings),
