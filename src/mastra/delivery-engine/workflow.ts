...
function gitignoreRuntimeArtifactContractEvidence({
  ...
  if (/\bgenerated secrets?\b/i.test(criterion)) {
    requiredGroups.push({
...
function honoWorkerEntrypointContractEvidence({
  criterion,
  repoPath,
  task,
}: {
  ...
  const gaps: string[] = [];
  if (!/import\s*\{?\s*Hono\s*\}?\s*from\s*['"]hono['"]/i.test(source)) {
    gaps.push('src/index.ts must import Hono from hono.');
  }
  if (!/\bnew\s+Hono\b/.test(source)) {
    gaps.push('src/index.ts must create a Hono app.');
  }
...
function acceptanceCriterionEvidence({
  ...
  const tsconfigEvidence = tsconfigWorkerContractEvidence({ criterion, repoPath, task });
  if (tsconfigEvidence) return tsconfigEvidence;
