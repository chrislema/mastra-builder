...
export function releaseGateEvidenceCommandPlan(repoPath: string, persistTo?: string): ReleaseGateEvidenceCommand[] {
  ...
  if (databaseName && existsSync(join(resolve(repoPath), 'migrations'))) {
    ...
    if (releaseGateTranscriptFixtureAvailable(repoPath)) {
      ...
      const versionAuditSql = releaseGateTranscriptVersionAuditSql();
      commands.push(
        {
          tier: 'api',
...
- New Worker config must use compatibility_date "${currentWorkerCompatibilityDate()}" unless the source docs explicitly require a different recent date.
