...
export const workerScaffoldAcceptanceCriteria = {
  ...
  tsconfigWorker:
    'tsconfig.json exists with Cloudflare Worker TypeScript scaffold settings and includes worker-configuration.d.ts generated by Wrangler.',
...
function packageScriptContractEvidence(context: AcceptanceContractContext) {
  ...
  const scriptMatch = context.criterion.match(
    /\bscripts\.([A-Za-z0-9:_-]+)\b[\s\S]{0,80}\bexactly(?:\s+as)?\s+["']([^"']+)["']/i,
  );
